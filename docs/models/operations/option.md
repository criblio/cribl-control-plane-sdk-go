...
```go
criblcontrolplanesdkgo.WithServerURL("https://api.example.com")
```
...
```go
criblcontrolplanesdkgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
criblcontrolplanesdkgo.WithClient(httpClient)
```
...
```go
criblcontrolplanesdkgo.WithSecurity(/* ... */)
```
...
```go
criblcontrolplanesdkgo.WithSecuritySource(/* ... */)
```
...
```go
criblcontrolplanesdkgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
criblcontrolplanesdkgo.WithTimeout(30 * time.Second)
```
