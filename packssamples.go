...
func newPacksSamples(rootSDK *CriblControlPlane, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *PacksSamples {
	return &PacksSamples{
		rootSDK:          rootSDK,
...
func (s *PacksSamples) Get(ctx context.Context, id string, pack string, opts ...operations.Option) (*operations.GetOutputSystemSamplesByPackAndIDResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/p/{pack}/system/outputs/{id}/samples", request, nil)
	if err != nil {
	...
	res := &operations.GetOutputSystemSamplesByPackAndIDResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			if o.SkipDeserialization == nil || !*o.SkipDeserialization {
				...
				var out components.CountedOutputSamplesResponse
				if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
					return nil, err
				}

				res.CountedOutputSamplesResponse = &out
			}
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
...
func (s *PacksSamples) Create(ctx context.Context, id string, pack string, outputTestRequest components.OutputTestRequest, opts ...operations.Option) (*operations.CreateOutputSystemTestByPackAndIDResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/p/{pack}/system/outputs/{id}/test", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.CreateOutputSystemTestByPackAndIDResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			if o.SkipDeserialization == nil || !*o.SkipDeserialization {
				...
				var out components.CountedOutputTestResponse
				if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
					return nil, err
				}

				res.CountedOutputTestResponse = &out
			}
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
