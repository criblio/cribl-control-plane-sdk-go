...
func main() {
	...
	if err != nil {
		...
		if existingRoutes.ID != "" {
			_, err = client.Routes.Append(ctx, existingRoutes.ID, []components.RouteConfInput{newRoute}, operations.WithServerURL(packURL))

			if err != nil {
				log.Printf("Error appending Route in Pack: %v", err)
			} else {
				fmt.Printf("✅ Route added: my-route in Pack: %s\n", PACK_ID)
			}
		}
	}
