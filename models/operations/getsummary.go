// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/criblio/cribl-control-plane-sdk-go/models/components"
)

type GetSummaryRequest struct {
	// Filter for limiting the response by Cribl product: Cribl Stream (<code>worker</code>) or Cribl Edge (<code>managed-edge</code>).
	Mode *components.WorkerTypes `queryParam:"style=form,explode=true,name=mode"`
}

func (g *GetSummaryRequest) GetMode() *components.WorkerTypes {
	if g == nil {
		return nil
	}
	return g.Mode
}

type GetSummaryResponse struct {
	HTTPMeta components.HTTPMetadata `json:"-"`
	// a list of DistributedSummary objects
	CountedListDistributedSummary *components.CountedListDistributedSummary
}

func (g *GetSummaryResponse) GetHTTPMeta() components.HTTPMetadata {
	if g == nil {
		return components.HTTPMetadata{}
	}
	return g.HTTPMeta
}

func (g *GetSummaryResponse) GetCountedListDistributedSummary() *components.CountedListDistributedSummary {
	if g == nil {
		return nil
	}
	return g.CountedListDistributedSummary
}
