// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package components

import (
	"errors"
	"fmt"
	"github.com/criblio/cribl-control-plane-sdk-go/internal/utils"
)

type SearchQueueLengthType string

const (
	SearchQueueLengthTypeNumber SearchQueueLengthType = "number"
	SearchQueueLengthTypeStr    SearchQueueLengthType = "str"
)

type SearchQueueLength struct {
	Number *float64 `queryParam:"inline,name=searchQueueLength"`
	Str    *string  `queryParam:"inline,name=searchQueueLength"`

	Type SearchQueueLengthType
}

func CreateSearchQueueLengthNumber(number float64) SearchQueueLength {
	typ := SearchQueueLengthTypeNumber

	return SearchQueueLength{
		Number: &number,
		Type:   typ,
	}
}

func CreateSearchQueueLengthStr(str string) SearchQueueLength {
	typ := SearchQueueLengthTypeStr

	return SearchQueueLength{
		Str:  &str,
		Type: typ,
	}
}

func (u *SearchQueueLength) UnmarshalJSON(data []byte) error {

	var number float64 = float64(0)
	if err := utils.UnmarshalJSON(data, &number, "", true, nil); err == nil {
		u.Number = &number
		u.Type = SearchQueueLengthTypeNumber
		return nil
	}

	var str string = ""
	if err := utils.UnmarshalJSON(data, &str, "", true, nil); err == nil {
		u.Str = &str
		u.Type = SearchQueueLengthTypeStr
		return nil
	}

	return fmt.Errorf("could not unmarshal `%s` into any supported union types for SearchQueueLength", string(data))
}

func (u SearchQueueLength) MarshalJSON() ([]byte, error) {
	if u.Number != nil {
		return utils.MarshalJSON(u.Number, "", true)
	}

	if u.Str != nil {
		return utils.MarshalJSON(u.Str, "", true)
	}

	return nil, errors.New("could not marshal union type SearchQueueLength: all fields are null")
}

type WarmPoolSizeEnum string

const (
	WarmPoolSizeEnumAuto WarmPoolSizeEnum = "auto"
)

func (e WarmPoolSizeEnum) ToPointer() *WarmPoolSizeEnum {
	return &e
}

// IsExact returns true if the value matches a known enum value, false otherwise.
func (e *WarmPoolSizeEnum) IsExact() bool {
	if e != nil {
		switch *e {
		case "auto":
			return true
		}
	}
	return false
}

type WarmPoolSizeType string

const (
	WarmPoolSizeTypeNumber           WarmPoolSizeType = "number"
	WarmPoolSizeTypeWarmPoolSizeEnum WarmPoolSizeType = "warmPoolSize_enum"
)

type WarmPoolSize struct {
	Number           *float64          `queryParam:"inline,name=warmPoolSize"`
	WarmPoolSizeEnum *WarmPoolSizeEnum `queryParam:"inline,name=warmPoolSize"`

	Type WarmPoolSizeType
}

func CreateWarmPoolSizeNumber(number float64) WarmPoolSize {
	typ := WarmPoolSizeTypeNumber

	return WarmPoolSize{
		Number: &number,
		Type:   typ,
	}
}

func CreateWarmPoolSizeWarmPoolSizeEnum(warmPoolSizeEnum WarmPoolSizeEnum) WarmPoolSize {
	typ := WarmPoolSizeTypeWarmPoolSizeEnum

	return WarmPoolSize{
		WarmPoolSizeEnum: &warmPoolSizeEnum,
		Type:             typ,
	}
}

func (u *WarmPoolSize) UnmarshalJSON(data []byte) error {

	var number float64 = float64(0)
	if err := utils.UnmarshalJSON(data, &number, "", true, nil); err == nil {
		u.Number = &number
		u.Type = WarmPoolSizeTypeNumber
		return nil
	}

	var warmPoolSizeEnum WarmPoolSizeEnum = WarmPoolSizeEnum("")
	if err := utils.UnmarshalJSON(data, &warmPoolSizeEnum, "", true, nil); err == nil {
		u.WarmPoolSizeEnum = &warmPoolSizeEnum
		u.Type = WarmPoolSizeTypeWarmPoolSizeEnum
		return nil
	}

	return fmt.Errorf("could not unmarshal `%s` into any supported union types for WarmPoolSize", string(data))
}

func (u WarmPoolSize) MarshalJSON() ([]byte, error) {
	if u.Number != nil {
		return utils.MarshalJSON(u.Number, "", true)
	}

	if u.WarmPoolSizeEnum != nil {
		return utils.MarshalJSON(u.WarmPoolSizeEnum, "", true)
	}

	return nil, errors.New("could not marshal union type WarmPoolSize: all fields are null")
}

type SearchSettings struct {
	CompressObjectCacheArtifacts bool                      `json:"compressObjectCacheArtifacts"`
	FieldSummaryMaxFields        float64                   `json:"fieldSummaryMaxFields"`
	FieldSummaryMaxNestedDepth   float64                   `json:"fieldSummaryMaxNestedDepth"`
	MaxConcurrentSearches        MaxConcurrentSearchesType `json:"maxConcurrentSearches"`
	MaxExecutorsPerSearch        float64                   `json:"maxExecutorsPerSearch"`
	MaxResultsPerSearch          float64                   `json:"maxResultsPerSearch"`
	MaxSearchDuration            string                    `json:"maxSearchDuration"`
	SearchHistoryMaxJobs         float64                   `json:"searchHistoryMaxJobs"`
	SearchHistoryTTL             string                    `json:"searchHistoryTTL"`
	SearchQueueLength            SearchQueueLength         `json:"searchQueueLength"`
	WarmPoolSize                 WarmPoolSize              `json:"warmPoolSize"`
	WriteOnlyProviderSecrets     bool                      `json:"writeOnlyProviderSecrets"`
}

func (s *SearchSettings) GetCompressObjectCacheArtifacts() bool {
	if s == nil {
		return false
	}
	return s.CompressObjectCacheArtifacts
}

func (s *SearchSettings) GetFieldSummaryMaxFields() float64 {
	if s == nil {
		return 0.0
	}
	return s.FieldSummaryMaxFields
}

func (s *SearchSettings) GetFieldSummaryMaxNestedDepth() float64 {
	if s == nil {
		return 0.0
	}
	return s.FieldSummaryMaxNestedDepth
}

func (s *SearchSettings) GetMaxConcurrentSearches() MaxConcurrentSearchesType {
	if s == nil {
		return MaxConcurrentSearchesType{}
	}
	return s.MaxConcurrentSearches
}

func (s *SearchSettings) GetMaxExecutorsPerSearch() float64 {
	if s == nil {
		return 0.0
	}
	return s.MaxExecutorsPerSearch
}

func (s *SearchSettings) GetMaxResultsPerSearch() float64 {
	if s == nil {
		return 0.0
	}
	return s.MaxResultsPerSearch
}

func (s *SearchSettings) GetMaxSearchDuration() string {
	if s == nil {
		return ""
	}
	return s.MaxSearchDuration
}

func (s *SearchSettings) GetSearchHistoryMaxJobs() float64 {
	if s == nil {
		return 0.0
	}
	return s.SearchHistoryMaxJobs
}

func (s *SearchSettings) GetSearchHistoryTTL() string {
	if s == nil {
		return ""
	}
	return s.SearchHistoryTTL
}

func (s *SearchSettings) GetSearchQueueLength() SearchQueueLength {
	if s == nil {
		return SearchQueueLength{}
	}
	return s.SearchQueueLength
}

func (s *SearchSettings) GetWarmPoolSize() WarmPoolSize {
	if s == nil {
		return WarmPoolSize{}
	}
	return s.WarmPoolSize
}

func (s *SearchSettings) GetWriteOnlyProviderSecrets() bool {
	if s == nil {
		return false
	}
	return s.WriteOnlyProviderSecrets
}
