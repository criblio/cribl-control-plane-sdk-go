...
type Input struct {
	...
	InputExecUnion            *InputExecUnion            `queryParam:"inline,name=Input"`
	...
	InputFileUnion            *InputFileUnion            `queryParam:"inline,name=Input"`
	...
	InputJournalFilesUnion    *InputJournalFilesUnion    `queryParam:"inline,name=Input"`
...
func CreateInputExec(exec InputExecUnion) Input {
...
func CreateInputFile(file InputFileUnion) Input {
...
func CreateInputJournalFiles(journalFiles InputJournalFilesUnion) Input {
	typ := InputUnionTypeJournalFiles

	return Input{
		InputJournalFilesUnion: &journalFiles,
		Type:                   typ,
	}
}
...
func (u *Input) UnmarshalJSON(data []byte) error {
	...
	case "eventhub":
		...
		return nil
	case "exec":
		inputExecUnion := new(InputExecUnion)
		if err := utils.UnmarshalJSON(data, &inputExecUnion, "", true, nil); err != nil {
			return fmt.Errorf("could not unmarshal `%s` into expected (Type == exec) type InputExecUnion within Input: %w", string(data), err)
		}

		u.InputExecUnion = inputExecUnion
	...
	case "syslog":
		...
		return nil
	case "file":
		inputFileUnion := new(InputFileUnion)
		if err := utils.UnmarshalJSON(data, &inputFileUnion, "", true, nil); err != nil {
			return fmt.Errorf("could not unmarshal `%s` into expected (Type == file) type InputFileUnion within Input: %w", string(data), err)
		}

		u.InputFileUnion = inputFileUnion
	...
	case "raw_udp":
		...
		return nil
	case "journal_files":
		inputJournalFilesUnion := new(InputJournalFilesUnion)
		if err := utils.UnmarshalJSON(data, &inputJournalFilesUnion, "", true, nil); err != nil {
			return fmt.Errorf("could not unmarshal `%s` into expected (Type == journal_files) type InputJournalFilesUnion within Input: %w", string(data), err)
		}

		u.InputJournalFilesUnion = inputJournalFilesUnion
...
func (u Input) MarshalJSON() ([]byte, error) {
	...
	if u.InputExecUnion != nil {
		return utils.MarshalJSON(u.InputExecUnion, "", true)
	}
	...
	if u.InputFileUnion != nil {
		return utils.MarshalJSON(u.InputFileUnion, "", true)
	}
	...
	if u.InputJournalFilesUnion != nil {
		return utils.MarshalJSON(u.InputJournalFilesUnion, "", true)
	}
